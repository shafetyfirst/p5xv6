...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT){
    ...
    if (ip->type == T_CHECKED){
          ...
          //cprintf("BMAP ADDRESS: %x\n", addr); 
          //cprintf("BMAP BN: %d\n", bn);     
          //checksum = checksum >> 24;
          //cprintf("2bmap addr: %x\n", addr);
          //cprintf("2bmap checksum: %x\n", checksum);
          /*
          if((addr >> 24) != checksum){
            cprintf("bad checksum\n");
            return -1;
...
{
  uint tot, m, checksumA, checksumB, temp, *a;
  struct buf *bp;
  int i;  
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    //bp = bread(ip->dev, bmap(ip, off/BSIZE));
	  if(ip->type == T_CHECKED){
        ...
        brelse(bp);
        //cprintf("read BN: %d\n", off/BSIZE - NDIRECT);
        //cprintf("indirect address: %x\n", a[off/BSIZE - NDIRECT]);
        checksumA =  a[off/BSIZE - NDIRECT] >> 24;
        if(off/BSIZE - NDIRECT == 127) checksumA = bmap(ip, off/BSIZE) >> 24;
        bp = bread(ip->dev, a[off/BSIZE - NDIRECT] & 0x00FFFFFF);
      } else{
		    bp = bread(ip->dev, ip->addrs[off/BSIZE] & 0x00FFFFFF);
        checksumA = ip->addrs[off/BSIZE] >>24;
	    }
      checksumB = bp->data[0];
	   	for(i = 1; i < 512; i++){
	       		temp = bp->data[i];		
