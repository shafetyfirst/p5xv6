...
stati(struct inode *ip, struct stat *st)
{
  ...
  uchar checks;
  int i;
  for(i = 0; 0 < NDIRECT + 1; i++){
     checks = (ip->addrs[i] & 0x1000) ^ checks;
     cprintf("checksum: %x\n", checks);
  }	
  st->checksum = checks;
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    int stuff = bmap(ip, off/BSIZE);
    cprintf("map: %d\n", stuff);
    bp = bread(ip->dev, stuff);
    m = min(n - tot, BSIZE - off%BSIZE);
    ...
    if(ip->type == T_CHECKED){	
  	...
  	ip->addrs[off/BSIZE] = (ip->addrs[off/BSIZE]) & 0x0111;
